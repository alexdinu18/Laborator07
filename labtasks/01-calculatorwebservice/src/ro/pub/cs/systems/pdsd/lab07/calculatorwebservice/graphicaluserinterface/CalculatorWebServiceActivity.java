	...
	private class CalculatorWebServiceThread extends Thread {
		...
		public void run() {
			...
			if (op1 == null || op1.isEmpty()) {
		        errorMessage = "Operator 1 cannot be empty";
		    }
			else if (op2 == null || op2.isEmpty()) {
		        errorMessage = "Operator 2 cannot be empty";
		    }
			...
			// 2. POST
			else if(methodsSpinner.getSelectedItemPosition() == Constants.POST_OPERATION) {
			...
			final String finalizedResult = content;
			resultTextView.post(new Runnable() {
				@Override
				public void run() {
